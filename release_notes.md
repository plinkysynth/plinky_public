...
this file serves as a log of changes to the firmware. with many thanks to RJ, who has been providing most of the recent fixes to the firmware, and provided the inaugural B0 notes. 

From newest to oldest:

# v0.B2 - Add Plinky+ detection, general improvements

## Plinky+ detection
- Plinky firmware can now detect whether it's running on a Plinky or a Plinky+
...
- Latched notes are no longer muted when a shift-button is pressed
- Midi notes now also play correctly when they come in almost simultaneously (specifically when sent on the same step by an external sequencer)
- The pressure calculation from CV gate is simplified: 0 - 5V now maps linearly to 0 - 100% pressure
- When Plinky reverts from external clock (euro or midi) to internal because the external clock is no longer present, the sequencer now stops playing
- When Plinky is following its internal clock, the first incoming euro clock will now reset and play the sequencer
...
- Small bugfix for edge cases where the substep timing was not calculated accurately

# v0.B0 - Sequencer, Latch and Midi rewrite
## Deletions
...
## CV Gate
- CV gate is processed after touches, sequencer and midi are handled. Whatever pressure is the result of those gets multiplied by the CV gate voltage
...
- Substeps are now fully implemented in both step and live recording modes. For each step, the sequencer has four substeps for touch position (pitches) and eight substeps for pressure - evenly divided over the step
  ...
  - In step-record mode, lifting your finger after holding a pad for longer than the step-length will remember the last(!) step-length of touches before you lifted your finger
